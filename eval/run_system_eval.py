...
def summarize(results: list[dict[str, Any]]) -> None:
    ...
    for category, metrics in accuracy["categories"].items():
        print(
            f"- {category}: strict={metrics['strict']:.3f} "
            f"partial_credit={metrics['partial_credit']:.3f} n={metrics['n']}"
        )

    print()
    overall = accuracy["overall"]
    print(
        f"Overall accuracy: strict={overall['strict']:.3f} "
        f"partial_credit={overall['partial_credit']:.3f} n={overall['n']}"
    )
...
def calculate_accuracy(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for category in sorted(grouped):
        ...
        categories[category] = {
            ...
            "partial_credit": partial_credit_accuracy,
            "n": len(items),
        }
...
def evaluate_questions(
    ...
    for index, question in enumerate(questions, start=1):
        ...
        print(
            f"{prefix}[{index}/{len(questions)}] {result['question_id']} "
